...
**实时《逃离塔科夫》武器装配模拟器，搭载 EvoErgo 引擎与社区平台**
...
EFTForge 是一个全栈《逃离塔科夫》武器装配模拟器。它提供递归配件树模拟、基于 EvoErgo 引擎的实时属性计算、冲突检测，以及社区装配方案发布平台。所有物品数据均通过 [tarkov.dev](https://tarkov.dev) GraphQL API 实时获取。
...
- 递归配件树渲染，完整解析插槽与允许物品
- 出厂预设配件自动安装模拟
- 实时属性计算：人机工程学、后坐力、重量、EED、手臂耐力、过摆
- 完整弹匣装弹重量建模
- 真实配件冲突检测（`conflictingItems` + `conflictingSlotIds`）
- 非阻断式冲突提示通知
- 通过 LZ-String 压缩的可分享装配代码
- 页面刷新后自动恢复上次装配进度
...
**浏览器路径** - 启动器会自动打开浏览器标签页。默认路径指向 Windows 上的 Chrome。如果你使用其他浏览器，请修改以下这行：
...
后端控制台显示 **"Application startup complete"** 后，网站将自动加载武器列表。就这些 - 你已经在运行了。

> **注意：** `sync_tarkov_dev.py` 由启动器自动调用。本地开发时请勿直接运行该脚本 - 它仅用于生产服务器上的手动计划外数据重同步。
...
EvoErgo 理论体系由 **SpaceMonkey37** 原创提出。EFTForge 在其基础上实现并扩展了这一系统。没有 SpaceMonkey37 的基础理论，本项目将无从实现。
...
EFTForge 是一个粉丝自制项目，与 Battlestate Games 官方无任何关联。所有游戏数据均来源于 [tarkov.dev](https://tarkov.dev)。
