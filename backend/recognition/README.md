...
Across the three saved screenshots, 21 cards left ambiguous by OCR were checked by hand against the crops. The current matcher supports 17 of them correctly and none incorrectly. All four abstentions are cards whose icon shows the part with its own children fitted, a scope sitting in its mount or a receiver extension carrying a stock, which no bare reference can be placed against; `no_variant_explains_the_icon` is the recorded reason in each case. The previous shared-short-name matcher supported 15 of the same 21. That is one hand-checked set on one UI language at 1920x1080, not a benchmark.
